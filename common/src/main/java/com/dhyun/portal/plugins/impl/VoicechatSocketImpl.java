...
public class VoicechatSocketImpl extends VoicechatSocketBase implements VoicechatSocket {
    ...
    public void open(int port, String bindAddress) throws Exception {
        ...
        try {
            ...
            Teleport.LOGGER.error("Failed to parse bind IP address '{}'", bindAddress);
            e.printStackTrace();
            Teleport.LOGGER.info("Binding to wildcard IP address");
        }
        ...
        try {
            try {
                ...
                Teleport.LOGGER.error("Failed to bind to address '{}', binding to wildcard IP instead", bindAddress);
                socket = new DatagramSocket(port);
            }
        } catch (BindException e) {
            Teleport.LOGGER.error("Failed to run voice chat at UDP port {}, make sure no other application is running at that port", port);
            Teleport.LOGGER.error("Voice chat server error {}", e.getMessage());
            if (CommonCompatibilityManager.INSTANCE.isDedicatedServer()) {
                Teleport.LOGGER.error("Shutting down server");
                System.exit(1);
            }
            throw e;
        }
    ...
    private void checkCorrectHost() throws Exception {
        String host = Teleport.SERVER_CONFIG.voiceHost.get();
        if (!host.isEmpty()) {
            try {
                new URI("voicechat://" + host);
            } catch (URISyntaxException e) {
                Teleport.LOGGER.warn("Failed to parse voice host: {}", e.getMessage());
                System.exit(1);
                throw e;
            }
        }
