...
public class AudioRecorder {
    ...
    public static AudioRecorder create() {
        ...
        String recordingDestination = TeleportClient.CLIENT_CONFIG.recordingDestination.get();
        Path location;
    ...
    private String lookupName(UUID uuid) {
        ...
        String username = TeleportClient.USERNAME_CACHE.getUsername(uuid);
        if (username == null) {
            return "system-" + uuid;
        }
        return username;
    ...
    public void appendChunk(UUID uuid, long timestamp, short[] data) throws IOException {
        ...
        long threshold = TeleportClient.CLIENT_CONFIG.outputBufferSize.get() * 20L;
    ...
    private void send(Component msg) {
        ...
        if (player != null && mc.level != null) {
            ...
            Teleport.LOGGER.info(msg.getString());
        }
    }
    ...
    private RandomAccessAudio convertFiles(File[] files, Consumer<Float> progress) throws UnsupportedAudioFileException, IOException {
        ...
        for (int i = 0; i < audioSnippets.size(); i++) {
            ...
            if (audio == null) {
                ...
                Teleport.LOGGER.warn("Audio snippet {} has the wrong audio format.", snippet.getA().getName());
                continue;
            }
