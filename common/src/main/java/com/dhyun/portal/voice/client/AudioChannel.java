...
public class AudioChannel extends Thread {
    ...
    public AudioChannel(ClientVoicechat client, InitializationData initializationData, UUID uuid) throws NativeDependencyException {
        ...
        this.packetBuffer = new AudioPacketBuffer(TeleportClient.CLIENT_CONFIG.audioPacketThreshold.get());
        ...
        Teleport.LOGGER.info("Creating audio channel for " + uuid);
    }
    ...
    public void closeAndKill() {
        Teleport.LOGGER.info("Closing audio channel for " + uuid);
        stopped = true;
        queue.clear();
    ...
    public void run() {
        try {
            ...
            while (!stopped) {
                ...
                if (!packet.isFromClientAudioChannel() && lastSequenceNumber >= 0) {
                    ...
                    if (packetsToCompensate > 0) {
                        Teleport.LOGGER.debug("Compensating {}/{} packets ", packetsToCompensate >= 4 ? 0 : packetsToCompensate, packetsToCompensate);
                    }
            ...
            decoder.close();
            Teleport.LOGGER.info("Closed audio channel for " + uuid);
        }
    }
    ...
    private void writeToSpeaker(SoundPacket<?> packet, short[] monoData) {
        ...
        if (TeleportClient.USERNAME_CACHE.has(uuid)) {
            channelVolume = (float) TeleportClient.VOLUME_CONFIG.getPlayerVolume(uuid);
        } else if (packet.getCategory() != null) {
            channelVolume = (float) TeleportClient.VOLUME_CONFIG.getCategoryVolume(packet.getCategory());
        } else {
            channelVolume = (float) TeleportClient.VOLUME_CONFIG.getPlayerVolume(Util.NIL_UUID);
        }

        float volume = TeleportClient.CLIENT_CONFIG.voiceChatVolume.get().floatValue() * channelVolume;
