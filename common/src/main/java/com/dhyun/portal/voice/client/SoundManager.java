...
public class SoundManager {
    ...
    private long openSpeaker(@Nullable String name) throws SpeakerException {
        try {
            ...
            if (name != null) {
                Teleport.LOGGER.warn("Failed to open audio channel '{}', falling back to default", name);
            }
            try {
    ...
    public void runInContext(Executor executor, Runnable runnable) {
        ...
        executor.execute(() -> {
            ...
            if (diff > 20 || (diff >= 5 && Teleport.debugMode())) {
                Teleport.LOGGER.warn("Sound executor delay: {} ms!", diff);
            }
            if (openContext()) {
                runnable.run();
                closeContext();
    ...
    public static boolean checkAlError() {
        ...
        Teleport.LOGGER.error("Voicechat sound manager AL error: {}.{}[{}] {}", stack.getClassName(), stack.getMethodName(), stack.getLineNumber(), getAlError(error));
        return true;
    }
    ...
    public static boolean checkAlcError(long device) {
        ...
        StackTraceElement stack = Thread.currentThread().getStackTrace()[2];
        Teleport.LOGGER.error("Voicechat sound manager ALC error: {}.{}[{}] {}", stack.getClassName(), stack.getMethodName(), stack.getLineNumber(), getAlcError(error));
        return true;
    }
